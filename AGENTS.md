...
## Vendored Obsidian types

`types/obsidian.d.ts` is a verbatim copy of `node_modules/obsidian/obsidian.d.ts`, mapped in `tsconfig.json` via `paths`. It is committed deliberately.

Obsidian's plugin review checker type-checks the repo without the plugin's own `node_modules` installed. With no `obsidian` package to resolve, every value imported from it becomes an error type and typescript-eslint's `no-unsafe-*` rules fire on almost every line: 208 warnings, reproducible exactly by deleting `node_modules/obsidian` and linting. Committing the types makes the module resolvable from the repo alone, so the checker sees what a developer sees.

Keep it in step with the dependency:

```
npm run types:check    # fails if the copy has drifted
npm run types:sync     # re-copy from node_modules
```

Run `types:check` after bumping the `obsidian` dependency. The file is generated by Obsidian and MIT licensed; never hand-edit it.
